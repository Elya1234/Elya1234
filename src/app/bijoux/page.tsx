...
export default function BijouxIndexPage() {
  return (
    <>
      <Breadcrumb items={[{ label: "Accueil", href: "/" }, { label: "Joaillerie" }]} />
          ...
          {categories.map((c) => (
            <Link key={c.slug} href={`/bijoux/${c.slug}`} className="group">
              <div className="relative aspect-[4/5] overflow-hidden bg-gris-produit">
                <Image src="/placeholders/category-bagues.svg" alt="" fill sizes="(min-width:1024px) 30vw, 90vw" className="object-cover transition-transform duration-700 group-hover:scale-105" />
              </div>
